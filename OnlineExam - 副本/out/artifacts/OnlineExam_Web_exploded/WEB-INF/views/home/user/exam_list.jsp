	...
	<div class="tm_main">
        ...
        <div class="tm_container">
        	<div class="tm_navtitle">
				<h1>我的测试</h1>
                <span>我的测试，请选择以下列表中我需要参加的测试。</span>
            </div>
        </div>
						...
						<tr id="tr-${exam.id }" data-key="${exam.id }">
								...
								<td >
									<c:if test="${nowTime > exam.endTime.time}">
										<button type="button" class="tm_btn" >作答结束</button>
									</c:if>
									<c:if test="${nowTime < exam.endTime.time}">
										<button type="button" class="tm_btn tm_btn_primary" onclick="tm.startExam('tr-${exam.id }');">开始答题</button>
									</c:if>
								</td>
						</tr>
					</c:forEach>
...
<script type="text/javascript">
		...
		var tm = {
				startExam : function(e){
					...
					html.push('	<p style="line-height:20px">确定开始答题吗？</p>');
					...
					html.push('		<td><p><b>测试名称</b>：'+tr.find("td").eq(0).text()+'<p>');
					html.push('			<p><b>测试时长</b>：'+tr.find("td").eq(2).text()+'<p>');
